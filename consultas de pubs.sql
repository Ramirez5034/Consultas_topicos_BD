...
--Listar las categorías de libros junto con la cantidad total de copias vendidas por categoría, ordenadas por la cantidad total de copias vendidas en orden descendente
SELECT C.type, SUM(S.qty) AS CopiasVendidas
FROM titles AS T
LEFT JOIN sales AS S ON T.title_id = S.title_id
LEFT JOIN roysched AS R ON T.title_id = R.title_id
LEFT JOIN discounts AS D ON T.title_id = D.title_id
LEFT JOIN stores AS ST ON S.stor_id = ST.stor_id
LEFT JOIN categories AS C ON T.type = C.type
GROUP BY C.type
ORDER BY CopiasVendidas DESC;
